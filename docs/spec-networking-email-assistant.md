...
- **Approval routing**: inbound texts from 555-555-0123 only. Bare `send`/`skip` applies to the single most-recent `awaiting_approval` draft; if more than one is pending, the bot replies with a numbered list and requires `send <n>`. Any message containing an arXiv URL/ID triggers intake.
...
Install `imessage-kit`; grant Full Disk Access to the terminal/node binary. Send a test text to 555-555-0123; poll chat.db for the reply.
